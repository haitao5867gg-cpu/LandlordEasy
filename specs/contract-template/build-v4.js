...
const terms = table(k, [
  new TableRow({ children: [L('房屋地址', k[0]), cell('【房屋坐落】（整间出租）', W - k[0], { span: 3 })] }),
...
const clauses = [
  ...
  item('3.', '水、电采用预付充值方式：水费【水费计费规则】，电费【电费计费规则】；网络及其他费用【其他费用规则】。政府调整水、电价格的，甲方可以按调整幅度相应调整，并提前通知乙方。'),
  ...
  item('2.', '因乙方使用不当或过错造成损坏的，**维修费用由乙方承担，凭维修单据或双方确认的金额，在维修完成时当场支付**；无法修复的，按同类物品市场价格扣除合理折旧后赔偿。因乙方原因造成漏水等给他人造成损失的，由乙方赔偿。'),
  ...
  item('5.', '有下列情形之一的，乙方可以书面通知甲方解除合同，无需支付违约金，甲方应退还押金：（1）房屋存在危及人身安全的隐患，经乙方告知后甲方未及时处理；（2）房屋主体或甲方提供的设施损坏，经乙方报修后甲方未及时维修，致使乙方无法正常居住。'),
  item('6.', '合同提前终止的，租金按实际居住天数结算，乙方已付但未居住期间的租金，扣除乙方应付款项后，与押金一并退还。'),
...
const body = [
  ...
  handover,
  para('说明：', { before: 200, size: 20, after: 40 }),
  para('1. 未提供的物品在“数量”栏填“—”。', { size: 20, after: 40 }),
  para('2. 水、电为预付充值，入住时无需抄表。', { size: 20, after: 40 }),
  para('3. 乙方入住后3日内发现本清单未记录的问题，可以告知甲方补充记录。', { size: 20, after: 40 }),
  para('4. 甲方交付确认、乙方接收确认：随本合同电子签名一并确认。', { size: 20 }),
];
