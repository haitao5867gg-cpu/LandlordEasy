...
- [ ] 18.10 **租客端:已付款账单可点击查看详情(费用明细+支付记录)。**
> 背景:GasCan反馈"我付完钱之后,租客端这里我想要再从列表点回去就没办法点到订单详情页了",希望能看到"当时付了多少钱、什么时候付的、付的钱是怎么组成的、通过什么渠道付的"。经"一问一答直到确认"的设计过程确认:**复用现有 `/bills/:id/pay` 路由**(不新建独立详情页),扩充 `PayBill.vue` 现有 `status === 'PAID'` 分支的展示内容;后端 `GET /tenant/bills`(`tenant-api.service.ts` `getMyBills`)已经 `include: { items: true, payments: true }`,**不需要改动后端**。确认的展示范围:①费用明细(`bill.items`,逐项展示name+amount,参照房东端`BillDetail.vue`的费用明细展示风格);②支付记录列表(`bill.payments`,只展示`status === 'CONFIRMED'`的记录,金额+支付时间+渠道,渠道要映射成中文,多笔全部列出不只显示一笔);**明确不展示**:`confirmedBy`(哪位房东确认的,不暴露给租客)、非CONFIRMED状态的支付记录(测试/失败尝试)。同时改 `MyBills.vue`,让PAID状态的账单也能点击跳转(现在 `is-link` 和 `goPay()` 都硬编码只对`PENDING`/`OVERDUE`生效)。
> 进度(2026-08-31,已分派Kiro CLI,尚未验证完成): 需求已交给Kiro CLI(headless)按上述设计实现,要求只改`MyBills.vue`和`PayBill.vue`两个文件、不碰后端和路由配置,完成后独立跑`vue-tsc -b`+`build`+真实浏览器验证多种场景(2费用项+1笔微信支付、多笔历史现金支付全部列出、PENDING账单原有支付流程不受影响)。**Claude Code尚未复核Kiro的实现和验证结果**,这是当前会话最新的进行中任务,如果这段对话被压缩/中断,下一个会话应该先检查Kiro CLI的输出(脚本日志在scratchpad目录,若已丢失可重新指挥Kiro或直接看 `git diff` 判断有没有代码改动)、跑一遍独立验证,再决定是否勾选完成。
