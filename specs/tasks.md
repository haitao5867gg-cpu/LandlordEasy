...
可能的解决方向(尚未验证,留给下次继续):①问微签技术人员有没有更权威的"查询完整签署状态"接口或字段;②看`eachSign/create`返回的`data`里(目前只解析了`bId`/`shortCode`)有没有被忽略掉的其他字段;③退而求其次的方案——用PDF内容里"乙方签字"区域是否存在实际笔迹/图像来判断(不可靠,不推荐);④更保守的方案——`launcherSignRule`自动盖章功能本身要不要保留是可以重新讨论的,如果它是这个"部分完成也能下载"问题的根源,退回"发起方也走手动`getSignPage`盖章"未必更差。
- [ ] 19.12 **真实场景验证:用微签体验额度走一遍完整流程——生成二维码→真实扫码关注→房东发起签署→租客收到微信客服消息+短信→完成实名认证签署→跳转/轮询核实到位→房东端看到已签署PDF+租客自动绑定成功。**
> 进度: 核心链路已用测试记录taskId=4完整验证成功(详见19.11进度记录),但19.11有未决问题(签署完成判定误判风险),19.12暂不勾选,等19.11收尾后一起确认。
