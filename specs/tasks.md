...
## M19 合同电子签约 + 服务号关注引导(2026-08-31立项)
...
>
> **本里程碑同样分两批**:第一批(19.1~19.5)不依赖"服务号内嵌vs小程序跳转"这个还在等腾讯销售经理确认的细节,可以先搭好框架;第二批(19.6~19.8)需要该细节确认后才能真正对接腾讯电子签"创建签署流程"接口,且需要GasCan先在腾讯电子签后台把提供的合同文本建成模板、拿到模板ID。
...
- [ ] 19.2 **微信公众号能力扩展:带参数二维码生成接口 + 关注/扫描事件webhook处理框架 + 客服消息接口,复用现有 `WECHAT_MODE=mock|real`(这是微信平台自身能力,不是腾讯电子签,不需要新变量)。**
- [ ] 19.3 **腾讯电子签API封装框架:创建签署流程/查询状态/签署完成回调,新增 `ESIGN_MODE=mock|real` 双模式(这个才是腾讯电子签专属的开关),real部分按官方Essbasic API格式实现结构,暂不要求真实跑通(缺模板ID)。**
...
- [ ] 19.6 **真实对接:ESIGN_MODE切real,接入真实模板ID,调通腾讯电子签创建签署流程API。**
- [ ] 19.7 **签署完成回调真实验签 + 自动绑定租客账号(用关注/扫描事件拿到的openid)。**
- [ ] 19.8 **真实场景验证:用腾讯电子签体验额度(10份中的1~2份)走一遍完整流程——生成二维码→真实扫码关注→收到继续签约推送→完成签署→回调触发→房东端能看到已签署PDF+租客自动绑定成功。**
