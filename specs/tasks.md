...
> 进度记录(2026-08-31,仍未完成,阻塞未解除;checkbox之前被误标成[x],2026-09-04核实内容后改回未完成,如实反映"资质未批、真实凭证未接入"的真实状态): 支付宝当面付资质提交审核后一直未获批,GasCan账号仍处于审核中状态,代码框架(mock/real双实现)已在18.4/18.5就绪,`ALIPAY_MODE` 独立开关也已就绪,等资质审核通过、GasCan拿到真实APPID+密钥后可以直接开工,不需要额外设计。**2026-09-04发现衍生问题**:生产环境`ALIPAY_MODE`/`PAYMENT_MODE`均未设置,`payments.module.ts`按约定退到默认`mock`,导致租客端`PayBill.vue`选"支付宝"时会真实显示"模拟支付成功(测试用)"按钮——**任何真实租客都能借这条路径把真实账单标记成已付而不实际付款**,是真实存在的生产资金安全问题,不是假设。GasCan要求先隐藏支付宝入口,处理见20.7。
...
- [x] 20.7 **堵住支付宝mock模式生产资金漏洞(隐藏支付宝入口)+新签租约表单4项体验优化。**
> 完成说明(2026-09-04):
> 1. **发现经过**:整理待办清单给GasCan看时,顺带查了18.8勾选状态,发现checkbox被误标`[x]`但内容明确写着"仍未完成、阻塞未解除"(已改回`[ ]`,如实反映)。GasCan看到后要求"支付宝支付的功能暂时隐藏吧"。核实生产环境`ALIPAY_MODE`/`PAYMENT_MODE`均未配置,`payments.module.ts`按约定退到默认`mock`——**这不只是UX问题,是真实资金安全漏洞**:租客端`PayBill.vue`选"支付宝"时会真实显示"模拟支付成功(测试用)"按钮,任何真实租客都能借这条路径把真实账单标记已付而不实际付款。
> 2. **修复方案**:新增独立开关`ALIPAY_ENABLED`(默认关闭),跟`ALIPAY_MODE`(mock/real)完全解耦——前者控制功能对用户是否可见/可用,后者控制走mock还是真实API,不冲突dev环境将来继续用mock测试完整流程的需要。后端`payments.controller.ts`的`createAlipayOrder`+`payments.service.ts`同名方法双重拦截(Kiro额外发现`simulateSuccess`对ALIPAY渠道存量记录也需要同款拦截,补上了,比原始任务要求更完整);`tenant-h5`的`PayBill.vue`彻底移除支付宝入口相关UI/逻辑及死代码,历史支付宝支付记录展示不受影响。
> 3. **新签租约表单4项体验优化**(`apps/landlord-h5/src/views/leases/NewLease.vue`):押金随月租金自动同步(判断"是否手动改过押金",手动改过后不再联动)、车牌号自动转大写+格式提示、金额输入补`inputmode="decimal"`+显示"元"单位、附加费用金额默认值从`0`改为空。
> 4. **Kiro CLI实现,Claude Code独立复核**(不采信自述):重跑`tsc`(0错误)+`jest`(10套件101用例全过,含新增的ALIPAY_ENABLED拦截用例)+两端`vue-tsc`(0错误)+两端`build`成功;逐行审查`payments.service.ts`diff确认双层拦截逻辑正确;部署dev后**真实验证**——①用真实Chrome注入房东JWT打开"新签租约"表单,真机操作确认押金自动同步(改租金1800→1900,押金手动改成2500后保持2500不被覆盖)、车牌`yueA88888`自动转成`YUEA88888`、附加费用金额默认是空字符串(不是0);②直接在NestJS应用上下文里调`createAlipayOrder`,确认真实抛出400"支付宝支付暂未开放,请使用微信支付";③用真实tenant JWT打开一条真实待付账单(¥1500)的支付页,确认页面**只显示"微信支付",支付宝按钮完全消失**。
> 5. **踩的坑**:这次会话用的内置浏览器工具突然对dev.landlordeasy.cn的静态资源返回`ERR_BLOCKED_BY_CLIENT`(重开标签页、重启整个浏览器面板都没用,但用`curl`确认服务器端资源本身是正常200),换用另一个浏览器工具(`claude-in-chrome`,用户真实Chrome)解决,记录下来供下次遇到类似情况参考——先用curl排除服务器问题,再考虑换浏览器工具本身。

- [x] 20.8 **房租催缴模板消息字段名对齐微信真实模板(自动+手动催缴均受影响)+补landlord-h5的ALIPAY渠道中文映射+夜间自主模式对M19/M20做一轮完整真实回归测试。**
> 完成说明(2026-09-04夜间~09-05凌晨,GasCan休息前授权自主模式,用真实Chrome自行测试验证):
> 1. **发现并修复真实bug**:`reminders.service.ts`(每日9点自动催缴)和`bills.service.ts`(房东手动"催一下"/批量催缴)一直用`keyword1/2/3`占位字段名调用模板消息接口,跟微信公众号后台真实配置的"房租账单催缴通知"模板(用真实access_token查`get_all_private_template`核实,字段是`amount3`=金额/`time4`=账单周期/`thing5`=账单类型/`thing7`=房间名称/`time10`=到期时间)完全对不上,自M18真实支付上线(`WECHAT_MODE=real`)以来这两条催缴路径可能一直静默发送失败(PM2日志有`errcode`非0但没人盯)。已改成正确字段名,`tsc`0错误、`jest`10套件101用例全过(`bills.service.spec.ts`断言同步更新)。
> 2. **顺手补`apps/landlord-h5/src/utils/status.ts`的`paymentChannelMap`缺失的`ALIPAY`映射**(18.10发现的遗留小问题),之前渠道是支付宝的支付记录会显示裸英文"ALIPAY"。
> 3. **部署**:推送dev分支(commit `0272051`),`deploy/deploy.sh dev`首次因GitHub出网SSL超时在`git pull`步骤失败(`set -e`导致后续构建/重启全部没跑,退出码却因外层`| tail`管道被掩盖成0——这正是文档反复强调的"复合命令健康检查不能证明部署生效"的真实案例),重试一次成功,核对服务器`git log -1`哈希与本地一致、健康检查200、PM2 `unstable_restarts=0`。
> 4. **ALIPAY映射真实浏览器验证**:dev库当时没有真实ALIPAY支付记录,插入一条一次性测试记录(billId=274)验证`billDetail`页正确显示"支付宝",验证后立即删除。
...
## M22 新标准合同模板(2026-09-20深夜立项,GasCan要求次日上线启用,当晚通宵完成)

> 背景:GasCan提供《住房租赁合同正式标准模板.docx》要求替换系统原6条2页简版合同。业务口径经DECISIONS.md一问一答+当晚拍板全部确认,定稿文本见`specs/contract-template/FINAL-TEMPLATE.md`(实现唯一依据)。GasCan追加拍板:强制"先入住交接后生成签约";通知送达只写双方电话删送达地址;8个占位符做成全局配置;"房东检查提前通知"条款整条删除;钥匙/影像存档三字段整体不要;附件五费用文案待提供先占位。

- [x] 22.1 **Prisma schema:新增CoOccupant同住人模型;ContractSettings新增收款人/费用规则/默认物品清单/7个期限配置(全部带默认值)**
> 完成说明(ZCode,commit eedb9dc): 无损新增字段+新表,dev已db push。房东检查提前通知字段按GasCan追加拍板删除。
- [x] 22.2 **合同PDF模板整体重写(9条正文+5附件,固定8页,签署页恒为最后一页)**
> 完成说明(ZCode,eedb9dc): 沿用固定高度页+overflow hidden技术;物品清单固定12行打勾式、同住人独立一页固定8行(GasCan提议的防变长设计);附件三自动取CHECKIN交接记录、附件二取同住人、期限数值取ContractSettings;jest新增页数确定性断言(3组数据变体均精确8页);本地渲染逐页目检+真实微签PDF复验。
- [x] 22.3 **盖章坐标校准(微签x左起/y底部起算,第8页甲方240/710、乙方710/710)**
> 完成说明(ZCode,6505851): 首次设y=260误当顶部起算,taskId=19真实预览PDF像素级实测红章落在73.9%高(=1000-260)证实底部起算(原注释本就写明);修正后taskId=20复验红章中心(23.8%宽,28.9%高)精确压在甲方签署线上。两轮各消耗1份真实微签额度。
- [x] 22.4 **同住人管理:CRUD接口四条+租约详情卡片+新签表单随填(备案性质,合同附件二数据源)**
> 完成说明(ZCode,eedb9dc/7db404c): E2E实测新签时填"王同住/4321/手机号"→详情卡片正确展示;增删改均走LandlordGuard接口。
- [x] 22.5 **交接单打通:checklist加quantity、默认物品清单一键预填、强制先交接后生成签约**
> 完成说明(ZCode): E2E实测未交接时生成签约被400拦截(文案符合设计);预填默认清单11项带数量;合同附件三正确显示"空调×1:完好";水电气表底数沿用签约任务字段。
- [x] 22.6 **系统设置扩展+退租提前通知提醒+顺手修复(UTC时间/登录抢跑toast/mock二维码/批量建房刷新)**
> 完成说明(ZCode 847788e/82bfe78,后两项由GasCan本地Codex实现+ZCode独立复核): 合同签约设置页新增收款人/4项费用规则/7个期限参数/默认物品清单(文本域每行"名称 数量");租客端退租申请展示"应提前N日通知"提醒(仅提示不拦截);UTC直出改本地时区(E2E实测17:38正确);登录前不再抢跑公寓列表(实测冷启动无toast);mock二维码改1x1合法PNG(实测不再破图);批量建房楼栋随公寓切换刷新(实测切换后正确只显示当前公寓楼栋)。
- [x] 22.7 **全链路E2E验证(dev真实浏览器+真实微签额度2份)**
> 完成说明(ZCode): 配置收款人占秀英+默认清单→建测试公寓/楼栋/房→新签租约(含同住人)→未交接被拦截→入住交接(预填+数量)→生成签约PENDING_SCAN→模拟微信关注(带真实签名)→自动发起(新模板真实额度,请求体pageNum=8坐标生效)→CREATED→预览PDF(8页,章位正确,状态不误变)→人工确认SIGNED→签署时间本地时区正确。两组测试数据零残留清理(taskId=4/5证据完好)。当晚两次部署事故(AdminModule未导出致DI崩溃/tsbuildinfo增量缓存致dist缺模块)均当场发现修复,再次验证"真实boot"铁律。
> **尚未完成,等GasCan**: ①真人走完微签实名认证+签字(验证新回调token自动确认路径,建议GasCan本人测)②生产微签凭证(GasCan测试环境彻底走通后购买)→配置生产.env→等GasCan说"推"→合并main部署生产。
