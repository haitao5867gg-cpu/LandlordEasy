...
The state database is owner-only SQLite and atomically binds comment ID, job ID, and comment hash before work begins. A process lock and single-node expiring lease fail closed. Failed/dirty worktrees are retained; worker jobs are never automatically retried. Raw bounded output stays in the local state directory; result comments pass redaction and high-risk secret detection. Results never intentionally include file bodies or inherited environment values. Terminal intent is durably queued before the terminal Issue POST, and a restart drains terminal and wake outboxes before scanning new work. Ambiguous claimed/running work is terminalized as FAILED without replay. Terminal completion and immutable wake identity creation are one SQLite transaction. If the optional wake bridge is enabled, the authoritative terminal lifecycle is always written to the queue Issue first. The separate wake comment contains no evidence, prompt, command, path, environment, provider output, database value or production authorization.
