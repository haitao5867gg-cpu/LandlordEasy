...
- **生产导入日志里一条支出记录因源数据错误被跳过**：`expenses_clean.csv` 第386行"冰箱"记录的 `amount` 字段本身是文本而非数字（原始Excel列错位导致），脚本正确跳过未硬写脏数据，影响1条支出记录（涉及金额约千元级别，具体数值未知），如需补录需人工核实真实金额后手动创建，不属于脚本bug，详见 `specs/tasks.md` 12.1。
- **178条低置信度切分点待人工抽查**：两个库导入都各产生178条需要人工复核的租约切分点（日期缺口或押金变化幅度触发阈值），完整清单在服务器 `/opt/landlord-easy/import-prod-run2.log`，建议GasCan有空时抽查几条金额较大的房间确认切分是否符合实际记忆，不阻塞当前上线。
