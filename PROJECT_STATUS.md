...
5. `apps/server/src/reminders/reminders.service.ts`房租催缴模板消息字段名（`keyword1/2/3`）跟微信后台实际模板字段（`amount3/time4/thing5/thing7/time10`）对不上，疑似一直静默发送失败，已用`spawn_task`记录独立任务待处理
6. `apps/landlord-h5/src/utils/status.ts`的`paymentChannelMap`缺`ALIPAY`映射（M18.10发现的小遗漏，不阻塞）
