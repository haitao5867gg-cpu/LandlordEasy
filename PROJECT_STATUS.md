...
**【2026-09-25 合同模板V3(docx,仅文档)】**GasCan要求模板长期可用(用2年以上不频繁改)、合法站得住、借鉴其母亲旧纸质合同精简。产出 `specs/contract-template/v3-template.docx`(正文10条+1附件,字数约为v2一半),改动明细与法律风险修正见 `specs/contract-template/V3-NOTES.md`。**系统合同生成代码未改,线上仍为v2**;待GasCan审阅V3后再决定是否移植(需重校微签章坐标)。提醒:甲方(个人)与产权人(公司)的转租合同尚未签署。
