...
export async function downloadContract(taskId: number, preview = false): Promise<void> {
  const blob = await http.get(`/leases/contract-signing-tasks/${taskId}/${preview ? 'preview' : 'pdf'}`, {
    responseType: 'blob',
    timeout: 60000,
  }) as unknown as Blob;
  ...
  link.href = url;
  link.download = `contract-${taskId}-${preview ? 'preview' : 'signed'}.pdf`;
  link.rel = 'noopener';
