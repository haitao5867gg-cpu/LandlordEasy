...
// 行上,不对再调。
// 2026-09-03 用taskId=8的预览PDF二次校准:x=210时章中心落在"甲方签字(按手
// 印)"标签正上方,把标签盖住了;右移到280让章落在标签右侧的空白签字线上。
// 2026-09-20 M22 新合同模板(8页,签署页固定为第8页最后一页)。
// 注意y是从下往上:第一次设y=260(误当顶部起算),真实预览PDF(taskId=19)
// 像素级实测红章落在73.9%高度=1000-260=740,与"y从底部起算"精确吻合。
// 签署页两条签署横线中心约在29%高度(=71%从底部):甲方x=240(24%宽),
// 乙方x=710(71%宽),y均取710,章中心压在签署线上。
// 2026-09-22v2 合同压缩为5页(正文2+附件合页3,签署区与附件五同在第5页);
// y=489/页5:150dpi像素实测(签署线位于顶部51.1%,1000-511=489)。
...
export class RealWeiQianService implements IWeiQianService {
  ...
  async createEachSignTask(
    ...
    const data = {
      ...
      rType: '1',
      // authType=2=实名认证(身份证四要素+人脸活体)。2026-09-22 GasCan拍板
      // 从短信验证码(authType=1)切换:收押金租金的合同需要"本人签署"的
      // 硬证据链,人脸认证+微签留存的核验记录在纠纷时才是关键证据。
      // 注意:authType=2没有短信通知,租客从签署链接/二维码进入走人脸流程。
      authType: '2',
